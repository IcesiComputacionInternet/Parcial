...
public class UpdateDocumentTest {
    ...
    public void TestUpdate_WhenDocumentIsOnApprovedCantBeModified(){
        IcesiDocumentDTO documentToTestDTO = defaultDocumentDTO();
        documentToTestDTO.setStatus(IcesiDocumentStatus.APPROVED);
        IcesiDocument documentToTest = defaultDocument();
        documentToTest.setStatus(IcesiDocumentStatus.APPROVED);
        when(documentRepository.findById(documentToTestDTO.getIcesiDocumentId())).thenReturn(Optional.of(documentToTest));
        assertThrows(IcesiException.class,
                () -> documentService.updateDocument("2dc074a1-2100-4d49-9823-aa12de103e70",documentToTestDTO));
    }
