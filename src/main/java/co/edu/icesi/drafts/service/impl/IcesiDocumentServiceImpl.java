...
public class IcesiDocumentServiceImpl implements IcesiDocumentService {
    ...
    public List<IcesiDocumentDTO> createDocuments(List<IcesiDocumentDTO> documentsDTO) {
        List<IcesiErrorDetail> errors = addErrorTitleExist(documentsDTO);
        errors.addAll(addErrorUserNotFound(documentsDTO));
        throwsErrorsInCreateDocuments(errors);
        return addDocuments(documentsDTO);
    }

    private List<IcesiErrorDetail> addErrorTitleExist(List<IcesiDocumentDTO> documentsDTO){
        return new ArrayList<>(documentsDTO.stream()
    ...
    private List<IcesiErrorDetail> addErrorUserNotFound(List<IcesiDocumentDTO> documentsDTO){
        return new ArrayList<>(documentsDTO.stream()
                .filter(documentDTO -> userRepository.findById(documentDTO.getUserId()).isEmpty())
                .map(documentDTO -> new IcesiErrorDetail
                        ("ERR_404",
                                "User with Id: "+documentDTO.getUserId()+" not found"))
                .toList());
    }

    private void throwsErrorsInCreateDocuments(List<IcesiErrorDetail> errors){
        if(!errors.isEmpty()){
    ...
    private List<IcesiDocumentDTO> addDocuments(List<IcesiDocumentDTO> documentsDTO){
    ...
    private IcesiUser validateUser(UUID userId){
